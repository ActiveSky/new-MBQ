### flash-attn版本

flash_attn-2.8.3+cu12torch2.8cxx11abiTRUE-cp310-cp310-linux_x86_64

### 8b模型

```bash
ps -ef | grep main.py | grep -v grep

CUDA_VISIBLE_DEVICES=6 \
nohup \
python3 -W ignore main_quant.py \
    --config configs/internvl2/MBQ_search/my_8b_weight_only.yaml \
    > 8b_out_w2g64.log 2>&1  &


CUDA_VISIBLE_DEVICES=0 \
nohup \
python3 -W ignore main.py \
    --config configs/internvl2/Eval/my_eval.yaml \
    > 8b_eval_out_ocrbench_w2g128.log 2>&1  &

CUDA_VISIBLE_DEVICES=3 \
nohup \
python3 -W ignore main.py \
    --config configs/internvl2/Eval/my_eval.yaml \
    > 8b_eval_out_ocrbench_w2g32.log 2>&1  &


CUDA_VISIBLE_DEVICES=3 \
nohup \
python3 -W ignore main.py \
    --config configs/internvl2/Eval/my_eval_textvqa.yaml \
    > 8b_eval_out_textvqa_w2g32.log 2>&1  &
```

### 2b模型测试

CUDA_VISIBLE_DEVICES=2 \
nohup \
python3 -W ignore main_quant.py \
 --config configs/my-internvl2-2b/MBQ_search/2b_weight_only.yaml \
 > out.log 2>&1 &

CUDA_VISIBLE_DEVICES=2 \
nohup \
python3 -W ignore main.py \
 --config configs/my-internvl2-2b/Eval/eval.yaml \
 > eval_out.log 2>&1 &

### 子仓库下载

```bash
git submodule update --init --recursive

git submodule update --init --recursive 3rdparty/lmms-eval

git submodule update --init --recursive 3rdparty/LLaVA-NeXT

```

### 激活环境

conda activate mbq

### flash-attn安装

```bash
pip install --no-build-isolation --no-cache-dir flash-attn==2.8.3
```

## 记录

**结论**
你的判断基本成立，而且更准确地说，MBQ 不是从头设计的一套新结构，而是以 AWQ 为工程骨架做出来的多模态 fork。它不只是“看起来像”，而是连调度方式、逐层搜索方式、scale 应用方式都沿用了 AWQ 的模板。

**为什么这么判断**

- 调度层同构：在 quant_wrapper.py 里，awq 和 mbq 被放进同一个分发器；MBQ 只是在 quant_wrapper.py 开始多了 reweight、distort、loss_mode、wa_quant 这些额外开关。
- 入口层同构：AWQ 的 entry.py 和 MBQ 的 entry.py 都是“组 q_config -> 跑过程 -> 存或读 scale -> apply -> 伪量化”的同一条流水线；MBQ 只是多了 CPU/GPU 切换和联合量化分支。
- 核心搜索层同构：AWQ 的 pre_quant.py 和 MBQ 的 pre_quant.py 都是“抓第一层输入 -> 按层循环 -> 收集线性层输入特征 -> 搜索 scale -> 逐层应用”的同一模板。MBQ 在这个模板上额外加了多模态输入处理、梯度缓存、reweight 和 distort。
- 算法内核同构：两个 auto_scale.py 和 auto_scale.py 的主体都是 20 档网格搜索、state_dict 回滚、按 loss 选 best_ratio；MBQ 只是把 loss 改成了能区分视觉 token 和答案 token 的 mask-aware 版本。
- 基础模块同构：两边的 qmodule.py 和 qmodule.py 里的 ScaledActivation 是同一份实现，这说明不是“思路相似”，而是工程基元直接复用。

**MBQ 真正新增的东西**

- 额外文件：MBQ 多了 auto_scale_wa.py、auto_scale_distort.py、auto_scale_wa_distort.py，这些都是 AWQ 没有的分支。
- 额外量化后端：MBQ 的 quantizer.py 仍保留 weight-only 伪量化，但从 quantizer.py 开始新增了 pseudo_quantize_model_weight_act，再配合 auto_scale_wa.py 里的 WALinear.from_float，说明它已经扩展到 weight-activation 联合量化。
- 额外模型适配：MBQ 的 pre_quant.py 和 pre_quant.py 覆盖了更多 VLM 结构，并且加入了 pre_quant.py、pre_quant.py 这类多模态专用逻辑。

**怎么定性**

- 如果你说“MBQ 的文件结构和 AWQ 很像”，这是对的。
- 如果你说“MBQ 是 AWQ 的多模态 fork / 增强版”，这是最准确的。
- 如果你说“MBQ 只是 AWQ 改个名字”，那就不够准确，因为 MBQ 已经改了输入形态、损失定义、量化后端和模型适配。

**代价**
这种复用的代价是代码重复很高，AWQ 和 MBQ 的 bug 修复、模型适配、scale 逻辑很容易分叉，后续维护成本会明显上升。

如果你愿意，我可以继续把 AWQ 和 MBQ 做成一张逐文件对照表，或者直接画出它们的调用链差异图。

qmodule(awq)==qmodule(mbq)

## MBQ代码区别

### gpt-5.4-mini 的分析

我先把差异按“入口、校准目标、量化实现、支持模型”四个层次归纳一下，这样能直接看出 MBQ 不是 AWQ 的小改，而是把 AWQ 的单模态权重缩放流程扩展成了多模态、带重加权和带激活量化的版本。

从 diff 看，MBQ 不是对 AWQ 的小修补，而是把 AWQ 的“单模态、weight-only、基于答案 token 的层级缩放搜索”扩成了“多模态、可做 weight-activation quant、带重加权和失真传播模拟”的版本。核心框架还是 AWQ 的 layer-by-layer 搜索与 apply_scale，但目标函数、输入形态、支持模型和量化后端都变了。

**最关键的差异**

- 入口从 AWQ 的 prompt_tokens / prompt_mask，变成 MBQ 的 prompt_inputs / prompt_kwargs，并额外拆出 vision_mask 和 caption_mask。对应逻辑在 MBQ 的 pre_quant.py 里，说明它已经不再只面向纯文本。
- 目标函数从“只看答案 token 上的误差”变成“答案 token + 视觉 token 的可重加权误差”，而且支持 mae / mse 两种模式。这个变化主要在 MBQ 的 auto_scale.py 和后面的 loss 分支里。
- MBQ 增加了 GradCacheHook，先反向统计视觉和文本 token 的梯度强度，再用 attn / mlp 的中位数比值去调权重搜索的偏好。AWQ 没有这一步，这是 MBQ 最“方法论”上的扩展。
- MBQ 还加了 distort 路径，会把当前层量化后的误差继续传给后续层，用失真输入参与校准，模拟量化误差传播。AWQ 只做标准的逐层校准，不做这类额外扰动传播。
- MBQ 支持 weight-activation quantization：在 quantizer.py 里引入 WALinear 和 a_bit，把原来 AWQ 的权重量化流程升级成了权重+激活一起量化的入口。

**模型支持层面**

- MBQ 扩展了 get_blocks 和 move_embed，对 Qwen2VL、LlavaLlamaModel、LlavaQwen、InternVL 等多模态封装做了适配。
- 在 auto_scale.py 里还专门加了 Qwen2VLDecoderLayer 分支，并在归一化缩放那里补了 Qwen2RMSNorm 的处理，说明它是针对新模型结构补齐的。
- qmodule.py 基本没有实质差异，说明 MBQ 的变化主要不在底层算子，而在校准流程和量化策略编排上。

**一句话概括**

- AWQ 更像“文本模型的权重缩放校准器”。
- MBQ 更像“面向多模态模型的、带梯度重加权和激活量化能力的 AWQ 超集”。

### 进一步解释

你卡住的这句，核心不是“代码语法”，而是 MBQ 在 AWQ 之外多加了一条“先测敏感度，再拿敏感度去指导量化搜索”的流程。可以把它拆成 3 层来看。

1. 先理解 AWQ 在做什么  
   AWQ 的主流程很简单：对每一层，拿输入特征去试不同的 scale，找一个让量化前后输出误差最小的 scale。它本质上是“逐层搜索最佳缩放系数”，目标主要是让答案 token 的输出尽量不变。这个基线在 awq 的 pre_quant.py 和 awq 的 auto_scale.py 里。

2. MBQ 额外加了什么  
   MBQ 先做了一次反向统计，这就是你看到的 GradCacheHook，代码在 pre_quant.py。它做的事不是训练模型，而是“偷看”每一层对不同 token 区域有多敏感：

- 它在若干线性层上挂 backward hook。
- 反向传播时，hook 拿到的是该层输出的梯度，不是权重梯度。
- 再用 vision_mask 和 caption_mask 把梯度切成两部分：视觉 token 和文本 token。
- 分别计算这两部分梯度的平均绝对值，得到“这层对视觉 token 和文本 token 的敏感度”。

所以这一步的真实含义是：先估计“图像区域”和“文本区域”在每层里谁更重要，而不是直接改模型参数。

3. “attn / mlp 的中位数比值”是什么意思  
   这部分在 pre_quant.py 附近。代码先把各层的视觉梯度 / 文本梯度比值收集起来，然后分成 attention 组和 MLP 组，各自取一个中位数。这里用中位数，是为了避免少数极端层把比例拉歪。

更准确地说，它不是把这个比值直接当最终权重，而是拿它做“下限”：

- 对某一层，先算它自己的 vis / cap 比值。
- 再和该组的中位数比。
- 最终取较大的那个。

也就是说，MBQ 不是完全按某一层的局部噪声走，而是“局部比值 + 组内稳健基线”一起决定权重。

你可以把它理解成一个公式：

$$
r_{\text{layer}} = \max\left(\frac{\overline{g}_{vis}}{\overline{g}_{cap}}, \text{median}_{group}\right)
$$

然后在量化搜索时把视觉部分的损失乘上这个 $r_{\text{layer}}$。

4. 这个权重到底怎么用到 scale 搜索里  
   真正搜索 scale 的地方在 auto_scale.py。MBQ 把 AWQ 原来“只看答案 token”的 loss，扩成了“答案 token + 视觉 token”的双区域 loss。

如果有 vision_mask 和 caption_mask，它就会同时算两部分误差：

$$
L = L_{\text{caption}} + r \cdot L_{\text{vision}}
$$

这里 $r$ 就是前面算出来的 reweight_ratio。  
如果没开 reweight，或者没有 vision_mask，它就退回到 AWQ 风格，只看答案区域，行为基本兼容。

5. distort 分支在干什么  
   这也是 MBQ 比 AWQ 多的一步，代码在 pre_quant.py 和后面的 pre_quant.py 附近。

它的意思是：  
当前层校准完之后，不是把“干净输入”继续传给下一层，而是把“经过量化层后的输出”传下去。这样后面的层做校准时，看到的是已经带有前面量化误差的输入。

这一步的作用是模拟误差累积。  
AWQ 通常是逐层独立校准，MBQ 则更接近“把前面层的量化失真也算进去”。

6. wa_quant 是什么  
   MBQ 还支持权重 + 激活联合量化，不只是 weight-only。对应入口在 quantizer.py。  
   当 wa_quant 打开时，它会把普通 Linear 替换成 WALinear，这意味着它已经不是 AWQ 那种纯权重量化路线了，而是更完整的 weight-activation 路线。

一句话总结一下：

AWQ 是“只做逐层权重缩放，尽量让答案 token 的输出不变”。  
MBQ 是“先用反向梯度估计图像和文本的重要性，再把这个重要性反馈到逐层 scale 搜索里，还可以额外模拟失真传播，并支持激活量化”。

如果你愿意，我下一步可以把这条链路按函数调用顺序画成一张非常直观的流程图，直接对应到代码里的每个函数。

### 流程图

```mermaid
flowchart TD
  A[prompt_inputs + prompt_kwargs] --> B[process_input]
  B --> C[inputs / vision_mask / caption_mask]

  C --> D{reweight = True?}
  D -- Yes --> E[GradCacheHook 注册 backward hooks]
  E --> F[反向传播统计 vis / cap 梯度]
  F --> G[按层名分组: attn_list / mlp_list]
  G --> H[np.median 得到 attn_median / mlp_median]
  H --> I[当前层比值 = vis_avg_grad / cap_avg_grad]
  I --> J["reweight_ratio = max(当前层比值, 组中位数)"]
  D -- No --> K[reweight_ratio = None]

  J --> L[传入 auto_scale_block / _wa / _distort]
  K --> L
  L --> M[搜索候选 scale]
  M --> N[loss = caption_loss + r * vision_loss]
  N --> O[选出 best scale]
  O --> P[apply_scale]

  P --> Q{distort = True?}
  Q -- Yes --> R[量化后的 layer 输出作为下一层输入]
  Q -- No --> S[正常传递 next layer input]
```

这张图里最关键的是两条线：

- `reweight_ratio` 这条线负责“给视觉/文本误差分配权重”。
- `distort` 这条线负责“把前面层的量化误差继续传下去”。

---

### 中位数计算的细节

这里的“该组的中位数”，不是对 token 求中位数，而是对“同一类层”的一组标量比值求中位数。

对应代码在 pre_quant.py 到 pre_quant.py：

- 先给每一层算一个比值  
  $r_l = \frac{\text{vis\_avg\_grad}}{\text{cap\_avg\_grad}}$
- 然后按层名分组：
  - 名字里包含 `o_proj` 或 `wo` 的，放进 attention 组，进入 `attn_list`
  - 名字里包含 `down_` 或 `w2` 的，放进 MLP 组，进入 `mlp_list`
- 最后对这个列表直接用 np.median 求中位数。

np.median 的规则是：

- 如果元素个数是奇数，取排序后正中间那个
- 如果元素个数是偶数，取中间两个数的平均值

所以它本质上是“这一组所有层的梯度比值”的稳健中心值，不是按样本算，也不是按 token 算。

举个例子，假设 attention 组算出来的比值是：

$$[0.8, 1.0, 1.2, 3.0]$$

那么中位数就是：

$$\frac{1.0 + 1.2}{2} = 1.1$$

之后在每一层真正做 reweight 时，代码会用当前层自己的比值和这个组中位数比较，取较大的那个。也就是 pre_quant.py 到 pre_quant.py 里的逻辑：

$$r_{\text{layer}} = \max(r_{\text{current}}, r_{\text{group-median}})$$

如果你愿意，我可以继续把“为什么这里要用中位数而不是平均数”也给你解释清楚。

---

### 常见疑问：weight-only 量化为什么收集输出梯度而不是权重梯度？

**核心理解**：scale search 优化的是**输出激活误差**，而不是权重误差。

在 auto_scale.py 的 `_search_module_scale` 里，流程是：

```
scale → 量化权重 → 前向传播 → 比较 org_out 和 quant_out
```

loss 定义为 $\|\text{org\_out} - \text{quant\_out}\|$，定义在**输出空间**上。所以即使做 weight-only quantization，搜索目标仍然是输出层面的误差最小化。

**为什么用输出梯度来指导这个搜索？**

输出梯度 $\frac{\partial \mathcal{L}}{\partial \mathbf{h}^{(l)}}$ 的含义是：

> 如果在第 $l$ 层的某个 token 位置的输出上扰动 $\epsilon$，最终 loss 大约变化 $\epsilon \cdot \frac{\partial \mathcal{L}}{\partial \mathbf{h}^{(l)}}$

权重被量化后，输出 $\mathbf{h}^{(l)}$ 会产生扰动 $\Delta \mathbf{h}$。不同 token 位置对这个扰动的敏感度不同：

- 梯度幅度大的位置 → 扰动会被放大到最终 loss → 量化时必须优先保护
- 梯度幅度小的位置 → 对扰动不敏感 → 可以容忍更大误差

所以用输出梯度的幅度来加权输出误差，本质上是：

$$
\text{weighted\_loss} = \sum_{\text{token } t} \underbrace{\left\| \frac{\partial \mathcal{L}}{\partial \mathbf{h}_t} \right\|}_{\text{敏感度}} \cdot \underbrace{\|\Delta \mathbf{h}_t\|}_{\text{量化带来的输出扰动}}
$$

**那权重梯度能不能用？**

权重梯度 $\frac{\partial \mathcal{L}}{\partial W}$ 告诉你"哪些权重重要"，但它不直接告诉你"这些权重影响了哪些 token"。要把权重重要性映射回 token 空间需要额外链式展开（$\frac{\partial \mathcal{L}}{\partial W} = \frac{\partial \mathcal{L}}{\partial \mathbf{h}} \cdot x^T$），信息粒度和可解释性都不如直接用输出梯度按 token 拆分。

---

### 常见疑问：reweight_ratio 用到 scale 搜索里怎么理解？

**从代码看**（auto_scale.py MAE 模式）：

```
masked_diff_ans = |org_out - out| * ans_mask
masked_diff_vis = |org_out - out| * vis_mask
loss = (sum(ans_diff) + reweight_ratio * sum(vis_diff)) / (N_ans + N_vis)
```

等价形式：

$$
\mathcal{L} = \frac{1}{N_{\text{ans}} + N_{\text{vis}}} \left( \sum_{t \in \text{ans}} |\Delta \mathbf{h}_t| \;+\; r \cdot \sum_{t \in \text{vis}} |\Delta \mathbf{h}_t| \right)
$$

**直观理解**：用一个具体例子说明——

| 候选 scale | 答案区域误差 | 视觉区域误差 |
| ---------- | ------------ | ------------ |
| A          | 0.01         | 0.05         |
| B          | 0.03         | 0.01         |

- **AWQ（只看答案）**：选 A，因为答案误差更小
- **MBQ（reweight_ratio=3，视觉更需要保护）**：
  - A 的总 loss = 0.01 + 3×0.05 = 0.16
  - B 的总 loss = 0.03 + 3×0.01 = 0.06
  - **选 B**

reweight_ratio 的作用就是告诉 scale search："在最小化输出误差的时候，视觉区域的重要性是答案区域的 r 倍"。r 来自梯度统计——如果视觉 token 的梯度幅度更大，说明模型对视觉区域的输出扰动更敏感，必须更小心保护。

**物理类比**：把 scale search 想象成"在预算有限的情况下分配保护力度"——

- 量化一定会引入误差（预算有限）
- 输出梯度告诉你"哪里受伤会更疼"
- reweight_ratio 就是把"疼痛敏感度"编码进 loss，让搜索算法自动把误差分配到"不那么疼"的区域

---

## reshape 形状变化的实际案例

以 InternVL2-8B 权重 `attention.wo [4096, 4096]` 为例：

以 InternVL2-8B 的实际参数为例，走三个典型场景：

---

### 场景 1：Per-channel 量化（默认，最常见）

这是 `w_quantize_func` 在 `_search_module_scale` 中被调用时的默认行为：

```
权重: attention.wo  [out, in] = [4096, 4096]

q_group_size = -1  →  跳过
per_tensor = False  →  跳过

tensor.shape 保持 [4096, 4096]
              ↓
dim=1 上 4096 个元素共享一个 scale/zero
dim=0 方向有 4096 个 channel → 4096 组独立的 scale/zero
```

```
         dim=1 (4096 columns)
    ┌──────────────────────────────┐
 d  │ row 0 → scale_0, zero_0      │
 i  │ row 1 → scale_1, zero_1      │
 m  │ row 2 → scale_2, zero_2      │  ← 4096 行 = 4096 个 channel
 =  │  ...                          │
 0  │ row 4095 → scale_4095        │
    └──────────────────────────────┘
```

---

### 场景 2：分组量化（q_group_size=128）

你配置中 `q_group_size: 128`，权重同一个 channel 内被进一步拆分：

```
权重: attention.wo  [4096, 4096]
                       │
q_group_size = 128 → reshape(-1, 128)
                       ↓
reshape 后: [131072, 128]
            │        │
            │        └── 每组 128 个元素共享一个 scale/zero
            │
            └── 4096 × 4096 / 128 = 131072 组
```

```
 原始形状 [4096, 4096]:
 ┌────────────────────────────────┐
 │ channel 0:  [● ● ● ● ... ● ●] │  4096 个元素
 │ channel 1:  [● ● ● ● ... ● ●] │
 │ ...                            │
 │ channel 4095: [● ● ● ● ... ● ●]│
 └────────────────────────────────┘

 分组后 [-1, 128]:
 ┌────────┐
 │组0  (128)│ ← channel 0 的 [0:128]   → scale₀
 │组1  (128)│ ← channel 0 的 [128:256] → scale₁
 │ ...     │
 │组31 (128)│ ← channel 0 的 [3968:4096]
 │组32 (128)│ ← channel 1 的 [0:128]   → scale₃₂
 │ ...     │
 │131071组 │ ← channel 4095 的末 128
 └────────┘
         每组独立 scale/zero → 总共 131072 组
```

---

### 场景 3：Per-tensor 量化

```
权重: attention.wo  [4096, 4096]
                       │
per_tensor = True → reshape(1, -1)
                       ↓
reshape 后: [1, 16777216]
            │   │
            │   └── 全部 4096×4096=1670万 个元素展平为一行
            │
            └── 1 个 scale / 1 个 zero → 整张量共用一个量化参数
```

```
 [1, 16777216]  只有一行 → 一组 scale, zero
 ┌───────────────────────────────────...──┐
 │ ●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●...●│  全部 1670 万元素
 └───────────────────────────────────...──┘
                 ↑
        一个 scale, 一个 zero
```

---

### 场景 4：Activation per-token 量化

通过 `quantize_activation_per_token_absmax` 调用前先做了预处理：

```
激活值: [batch=1, seq_len=2048, hidden=4096]
              │
         外部先 view(-1, hidden)  ← 在调用 pseudo_quantize_tensor 之前
              ↓
输入:   [2048, 4096]
              │
         q_group_size=-1, per_tensor=False → 不 reshape
              ↓
量化:   [2048, 4096]  →  2048 个 token，每个 token 独立 scale/zero
```

```
         dim=1 (4096 hidden)
    ┌──────────────────────────────┐
    │ token 0 → scale_0, zero_0     │
    │ token 1 → scale_1, zero_1     │
 d  │ token 2 → scale_2, zero_2     │  ← 每行 = 每个 token 的 4096 维向量
 i  │ ...                           │
 m  │ token 2047 → scale_2047       │
 =  └──────────────────────────────┘
 0
```

---

### 总结

| 量化方式        | q_group_size | per_tensor | 输入形状      | reshape 后     | dim=0 含义      |
| --------------- | :----------: | :--------: | ------------- | -------------- | --------------- |
| per-channel     |      -1      |   False    | `[4096,4096]` | `[4096,4096]`  | 4096 个输出通道 |
| group-wise(128) |     128      |   False    | `[4096,4096]` | `[131072,128]` | 131072 个组     |
| per-tensor      |      -1      |    True    | `[4096,4096]` | `[1,16777216]` | 整张量          |
| per-token (act) |      -1      |   False    | `[2048,4096]` | `[2048,4096]`  | 2048 个 token   |

核心原则：**reshape 后的 dim=0 决定有多少组独立的量化参数，dim=1 是每组内共享参数的区间**。

---

## Scale 函数详解

### `scale_fc_fc`：FC → FC 之间的 scale 迁移

处理 `V_proj → O_proj`、`up_proj → down_proj` 等 FC→FC 连接。

**数学原理**：

- `h = fc1(x)` → `y = fc2(h)`
- 变换：`fc1' = fc1 / scale`（输出变小），`fc2' = fc2 * scale`（权重补偿）
- 结果：`y' = fc2'(fc1'(x)) = y`，输出不变，但激活范围被压缩

**为什么只对末尾 hidden 行操作？**

InternLM2 使用 fused QKV（Q、K、V 拼成一个大矩阵），`wqkv.weight` 形状为 `[3 × hidden, hidden]`：

```
┌─────────────────────────┐
│  Q 的权重 [hidden, hidden]   │  ← 第 0 ~ hidden-1 行
├─────────────────────────┤
│  K 的权重 [hidden, hidden]   │  ← 第 hidden ~ 2×hidden-1 行
├─────────────────────────┤
│  V 的权重 [hidden, hidden]   │  ← 第 2×hidden ~ 3×hidden-1 行（末尾）
└─────────────────────────┘
         ↑ 只有 V 的输出 → wo(O_proj)
```

`scale_fc_fc` 处理的是 V → O 这条路径，所以只操作 `wqkv` 末尾的 hidden 行（V 的部分）：

```python
fc1.weight[-scales.size(0):].div_(scales.view(-1, 1))  # 只取末尾 hidden 行
```

Q 和 K 没有直接连接到后继 FC，所以不需要这一步骤。

### `scale_ln_fcs`：LN → FC 之间的 scale 迁移

处理 `input_layernorm → QKV`、`ffn_norm → gate/up` 等 LN→FC 连接。

**Q、K、V 为什么使用同一套 scale？**

因为 Q、K、V 的输入来自同一个 LN 输出：

```
                input_layernorm
                      ↓
                 LN 输出 [hidden_dim]
                      ↓
        ┌─────────────┼─────────────┐
        ↓             ↓             ↓
     Q_proj        K_proj        V_proj
```

scale 是针对**共同输入**的每个 channel 设计的：

```python
ln.weight.div_(scales)           # LN 权重缩小 → 输出变小
for fc in [Q, K, V]:             # 所有 FC 的输入都变小了
    fc.weight.mul_(scales)       # 都需要放大权重来补偿
```

数学等价：`LN'(x) = LN(x)/scale` → `Q'(x') = Q(x)/scale * scale = Q(x)`（K、V 同理）。

### 搜索流程如何考虑架构

```python
# 搜索时：基于共同输入计算 scale
_auto_get_scale(
    prev_op=module.input_layernorm,
    layers=[module.self_attn.q_proj, module.self_attn.k_proj, module.self_attn.v_proj],
    inp=input_feat["self_attn.q_proj"],  # Q、K、V 输入相同，只传一个
)

# 应用时：
# 1. LN /= scale
# 2. Q *= scale, K *= scale, V *= scale（同一套）
```

### 两个函数对比

| 函数           | 处理位置 | 前驱类型          | 操作方式                                       |
| -------------- | -------- | ----------------- | ---------------------------------------------- |
| `scale_ln_fcs` | LN → FC  | LayerNorm/RMSNorm | LN.weight /= scale, FC.weight \*= scale        |
| `scale_fc_fc`  | FC → FC  | Linear            | fc1.weight[-N:] /= scale, fc2.weight \*= scale |


---

## 在尝试的Ideas



### 一、为什么你的 2-bit 效果这么差

2-bit 只有 4 个量化等级，核心矛盾是：**权重矩阵中的 outlier 会把 scale factor 拉得过大，导致绝大多数权重被压缩到同一个量化值，信息完全丢失**。

- `group_size=32` 能保留 ~80%：每组有自己的 scale，outlier 的影响被局部隔离
- `group_size=128` 近乎归零：一个 outlier 就毁掉 128 个权重，更多权重被"压平"

这只靠 AWQ 的 scale search 是不够的——scale search 只是在现有分布上找最优 scale，不能改变分布本身。2-bit 需要的是**从根本上改造量化前的权重分布**。

---

### 二、6 个可组合方法 + 可行性分析

按**实现成本从低到高**排列：

---

#### 方法 1：Rotation 预处理（来自 MQuant / QuaRot 线）

**原理**：在量化前对权重矩阵施加随机正交旋转 $Q_1 W Q_2$。正交矩阵会把 channel 维度的 outlier 打散到所有 channel，使权重分布更均匀、更接近高斯，大幅降低 2-bit 量化的难度。

**为什么适合 2-bit**：你 group_size=128 近乎归零的本质原因是 outlier 集中。Rotation 是已知最能解决 outlier 问题的单一步骤，不需要改量化器本身。

**与 MBQ 的结合方式**：在 MBQ 的 pre_quant.py 中，`get_blocks()` 获取 layer 之后、`search_scale()` 之前，插入一步 QuaRot 风格的旋转——对每个 Linear 做 $Q_1 W Q_2^{-1}$，同时在线修改上一层的权重以保持等价性（QuaRot 的 fuse 机制）。MQuant 更进一步做了 RMS（Rotation Magnitude Suppression）来压制旋转引入的新 outlier。

**可行性**：⭐⭐⭐⭐⭐ 最高。MQuant 代码在 MQuant 已收集，基于 QuaRot 实现，可以和 MBQ 的 AWQ 骨架直接叠加。LLM 领域有大量验证（QuaRot、QuIP、SpinQuant），VLM 领域 MQuant 已证明有效。

**预期收益**：2-bit + group_size=128 从接近 0 恢复到可用水平（MQuant 在 W4A8 下效果接近 FP16）。

**风险**：需要在每一层的输入/输出插入旋转矩阵的在线融合逻辑，对 attention 层的 QKV 投影需要特殊处理。

---

#### 方法 2：低秩分解 + 残差量化（来自 QSVD / WSVD 线）

**原理**：$W = AB + R$，其中 $AB$ 是低秩近似（保留主要结构），$R$ 是残差。$AB$ 用 4-bit 或 FP16 存储，残差 $R$ 用 2-bit 量化。因为去掉主成分后的残差 $R$ 幅值小、分布更均匀，2-bit 量化误差远小于直接量化 $W$。

**为什么适合 2-bit**：直接 2-bit 量化 $W$ 需要 4 个等级覆盖整个数值范围，但量化 $R$ 只需要 4 个等级覆盖残差范围——后者通常小一个数量级。

**与 MBQ 的结合方式**：
- 步骤 1：对 MBQ 找到的每个 Linear 做 SVD，得到 $U, S, V$
- 步骤 2：取前 $k$ 个奇异值构成低秩近似 $W_{LR} = U_k \Sigma_k V_k^T$，存储为两个小矩阵
- 步骤 3：残差 $R = W - W_{LR}$ 用 MBQ 的 2-bit pipeline 量化
- 步骤 4：推理时 $Y = X W_{LR} + X R_q$（或把 $W_{LR}$ 吸收到相邻层）

QSVD 的 `fake_quant/svd_utils.py` 已经实现了 QKV 联合分解 + GPTQ 量化的完整流程，可以直接参考。

**可行性**：⭐⭐⭐⭐。QSVD (QSVD) 和 WSVD (WSVD) 代码都在本地。QSVD 是 NeurIPS 2025 Spotlight，WSVD 是 ICLR 2026，两个都直接开源可用。关键是 QSVD 已经做了 "分解 + 量化" 的联合，你只需要把 MBQ 的 scale search 替换/叠加进去。

**预期收益**：大幅提升，QSVD 在 2-bit 级别有明显优势。

**风险**：推理时需要额外的矩阵乘法（两个小矩阵相乘），会增加少量延迟。需要对每个 Linear 独立决定 rank（QSVD 的 cross-layer rank allocation 已解决这个问题）。

---

#### 方法 3：混合精度分配（来自 MC# / MoPEQ 线）

**原理**：不是所有层都做 2-bit。根据每层的敏感度（Hessian trace、gradient norm、输出误差），敏感层（如首层、末层、attention Q/K/V 投影）保留 4-bit 或 8-bit，不敏感层用 2-bit。总平均 bit 宽度控制在 ~2.2-2.5 bit。

**为什么适合 2-bit**：你 group_size=32 已经有 80%，说明大部分信息还在，差的 20% 很可能是少数几个敏感层崩溃导致的。保护这 5%-10% 的关键层就能拉回大部分精度。

**与 MBQ 的结合方式**：最简单的是在 MBQ 入口加一个 `layer_bit_config` 字典，指定每层或每类模块的 bit 宽度。MC# 代码 (Mixture-Compressor-MoE) 提供了 expert-wise 的敏感度评分和 bit 分配逻辑，可以简化为 layer-wise。

**可行性**：⭐⭐⭐⭐⭐。这是最容易实现的，本质上是 MBQ 配置层面的一层 for 循环。MC# 已经支持 Mixtral 8×7B 做 expert-level 的 1.5-2.5 bit 混合精度。

**预期收益**：中等偏上。2-bit 平均值下精度可以接近 4-bit 的 90%+。

**风险**：部署时需要不同 bit 的解量化 kernel，增加了工程复杂度。论文角度容易被审稿人说"这只是调参，不是新方法"——需要配合敏感度分析的理论论证。

---

#### 方法 4：Modality-Aware Smoothing（来自 MASQuant 线）

**原理**：把 SmoothQuant 的 smooth 因子按 vision token 和 text token 分开学习，再加上 SVD whitening 把跨模态激活差异转化为低秩形式，让量化前的分布更均匀。

**为什么适合 2-bit**：MBQ 的核心发现就是 VLM 中视觉和文本模态的量化敏感度不同。但 MBQ 做的"平衡"是在 loss 层面（reweight），MASQuant 做的是在分布层面（smoothing）——后者对 2-bit 更根本，因为它直接降低量化难度。

**与 MBQ 的结合方式**：关闭 MBQ 的 reweight 和 distort，在 scale search 前加入 MASQuant 的 modality-aware smoothing（学习 vision/text 各自的 smooth 因子 + 可选 SVD whitening）。masquant 有完整实现。

**可行性**：⭐⭐⭐⭐。MASQuant 代码在本地，且它的 smoothing 逻辑和 MBQ 的 scale search 是正交的——一个改分布，一个找最优 scale，可以叠加。

**预期收益**：中等到较高。在文本和视觉 token 分布差异大的场景（如复杂 VQA）尤其有效。

**风险**：需要识别 VLM 中的 vision token 区域，不同模型架构的 token layout 不同，需要适配。

---

#### 方法 5：非均匀量化（来自 Bi-VLM 线 + OA-EM）

**原理**：AWQ/MBQ 用的是均匀量化（等距的 4 个 level）。但在 2-bit 下，最优的 4 个 level 通常不是均匀的——权重分布是类高斯或带厚尾的，需要非均匀的量化网格。

具体实现有两条路：
- **Bi-VLM 路线**：学习非均匀的量化 level（如 Lloyd-Max 量化器）
- **OA-EM 路线**：用 codebook 替代标量量化（2-bit = 4 个 codebook 向量），本质是向量量化

**为什么适合 2-bit**：均匀量化在 4-bit 以上还好，到了 2-bit 非均匀的优势急剧增大，因为可用 level 太少，每一个 level 的位置都至关重要。

**与 MBQ 的结合方式**：替换 MBQ 的 `pseudo_quantize_tensor` 中的 uniform quantizer 为 non-uniform quantizer（预学习或基于 K-means 的 codebook）。OA-EM 论文强调初始化比搜索更重要——可以直接用 K-means++ 初始化 codebook。

**可行性**：⭐⭐⭐。需要实现新的量化/解量化 kernel，部署时非均匀量化的 lookup table 会有额外开销。论文角度有创新性但实现成本较高。

**预期收益**：较高。理论上非均匀量化在 2-bit 下可以比均匀量化多保留 10-20% 的信息。

**风险**：非均匀量化的硬件支持不如均匀量化好，部署可能受限。OA-EM 代码未公开，需要自己实现。

---

#### 方法 6：校准数据优化（来自 Rethinking Calibration + LUQ + Best Practices）

**原理**：你现在的 MBQ calibration 可能用的是随机抽样的图片。但实际上：
- 应该优先选能覆盖视觉/文本模态差异的样本（Rethinking Calibration）
- 应该混合 image/text token 做 PTQ 采样（LUQ）
- 多模态 token 诱发的中间层激活方差更大，需要更好的校准覆盖（Best Practices / mmq）

**为什么适合 2-bit**：2-bit 对校准数据极度敏感——错误的 scale 在 4-bit 可能只损失 1%，在 2-bit 可能损失 20%。你 group_size=32 到 128 的巨大差异部分也可能源于校准数据不够有代表性。

**与 MBQ 的结合方式**：在 MBQ 的 entry.py 中替换 calibration 数据加载逻辑——不再随机抽样，而是做覆盖驱动的 sample selection（优先选高 variance、高 entropy 的样本）。mmq 做了大量 calibration 消融实验可以参考。

**可行性**：⭐⭐⭐⭐⭐。纯数据层面的改动，不改量化逻辑。实现成本最低。

**预期收益**：中等。能让 group_size 切换更平滑，但不会从根本上解决 2-bit 的信息瓶颈。

**风险**：效果有上限，不能替代方法 1-5 的结构性改进。

---

### 三、推荐的组合优先级

考虑到你是基于 MBQ 做 weight-only 2-bit，我的建议是按以下三层叠加：

| 层级 | 方法 | 理由 |
|------|------|------|
| **第一层（必做）** | Rotation 预处理（方法1）+ 混合精度（方法3） | Rotation 解决 outlier 集中问题，混合精度保护关键层，两者叠加最直接有效 |
| **第二层（推荐）** | Modality-Aware Smoothing（方法4）+ 校准优化（方法6） | 直接利用 MBQ 已有的多模态视角，改动小 |
| **第三层（深度）** | 低秩+残差量化（方法2）或 非均匀量化（方法5） | 结构性改进，需要更多工程投入，更适合做论文主体创新 |

**如果你只做一个**：选 Rotation（方法1），因为它在 2-bit 场景下的收益最确定、实现路径最清晰（MQuant 代码可参考）。

**如果你要做论文**：方法 2（QSVD 线）或 方法 1+4+3 的组合都有足够的创新空间。

---

## 已经验证的ideas



- Modality-Aware Smoothing（来自 MASQuant 线）实验过，效果不行
- 非均匀量化暂时不考虑
